...
Приложение способное считывать текстовое описание композиции в формате **ITMO Loops** и генерировать по нему `.wav` файл.
...
1. Программа читает файл с описанием композиции.

2. Строит внутреннюю модель трека (инструменты, эффекты, паттерны, временную структуру).

3. Генерирует одноканальный звуковой сигнал и записать корректный WAV-файл с [частотой дискретизации](https://en.wikipedia.org/wiki/Sampling_(signal_processing)#Sampling_rate) 44.1 kHz и [глубиной](https://en.wikipedia.org/wiki/Audio_bit_depth) 16 бит.
